/**
 * @file cas_proxy_task.cc
 * @brief Async CAS proxy task implementation.
...
class CasProxyTask::Impl : public std::enable_shared_from_this<Impl> {
 public:
  Impl(Executor io_executor, Executor callback_executor,
       bsrvcore::SslContextPtr ssl_ctx, CasProxyParams params)
  ...
  void Start() {
    ...
    std::string error_message;
    {
      ...
      request_url = detail::BuildProxyUrl(params_, &error_message);
  ...
  [[nodiscard]] const CasProxyParams& Params() const noexcept { return params_; }

 private:
  CasProxyResult MakeImmediateFailure(CasTaskStatus status,
                                      std::string error_message) const {
    CasProxyResult result;
    result.status = status;
    result.failure_message = std::move(error_message);
    return result;
  }

  CasProxyResult MakeCancelledResult() const {
    CasProxyResult result;
  ...
  void PostCompletion(CasProxyResult result) {
  ...
  void HandleHttpDone(const bsrvcore::HttpClientResult& result) {
    Complete(detail::ConvertProxyResult(result));
  }

  void Complete(CasProxyResult result) {
    Callback cb;
    {
      std::scoped_lock const lock(mutex_);
      if (completed_) {
...
std::shared_ptr<CasProxyTask> CasProxyTask::Create(Executor io_executor,
...
std::shared_ptr<CasProxyTask> CasProxyTask::Create(
...
std::shared_ptr<CasProxyTask> CasProxyTask::Create(
  ...
  return std::shared_ptr<CasProxyTask>(new CasProxyTask(std::move(impl)));
}

std::shared_ptr<CasProxyTask> CasProxyTask::OnDone(Callback cb) {
  impl_->OnDone(std::move(cb));
  return shared_from_this();
}

void CasProxyTask::Start() { impl_->Start(); }

void CasProxyTask::Cancel() { impl_->Cancel(); }

const CasProxyParams& CasProxyTask::Params() const noexcept {
  return impl_->Params();
}

CasProxyTask::~CasProxyTask() = default;

CasProxyTask::CasProxyTask(std::shared_ptr<Impl> impl)
    : impl_(std::move(impl)) {}
